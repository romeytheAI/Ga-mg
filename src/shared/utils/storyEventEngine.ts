...
import { DIALOGUE_TREES, DialogueChoice, DialogueTree } from '../../features/simulation/systems/dialogueTrees';
import { ActiveStoryEvent } from '../../core/types';
  ...
  if (choice?.end_dialogue) {
    return {
      ...
      parsedText: {
        ...
        follow_up_choices: choice.new_location ? (LOCATIONS[choice.new_location]?.actions || []).map(action => ({ ...action })) : [],

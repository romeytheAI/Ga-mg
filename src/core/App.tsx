...
import { SemanticText } from '../features/ui/components/TextComponents';
import { GameState, Item, StatKey, ActiveEncounter } from './types';
import { LOCATIONS } from '../features/world/locations/locations';
import { NPCS } from '../features/world/npcs/npcs';
import { BASIC_ITEMS } from '../features/character/clothing/items';
import { ENCOUNTERS } from '../features/simulation/encounters/encounters';
import { DIALOGUE_TREES } from '../features/simulation/systems/dialogueTrees';
import { initialState } from './state/initialState';
import { gameReducer } from './state/gameReducer';
import { PREDEFINED_ANATOMIES, STABLE_API, DEFAULT_API_KEY, AGE_APPEARANCE } from './constants';
import { ELDER_SCROLLS_LORE, getRelevantLore } from '../features/world/lore/lore';
import { generateText, generateImage, generateLegendaryStats } from '../shared/services/api';
import { buildTextPromptAsync, buildImagePrompt, getVisualEffectClasses, imageWorker } from '../shared/utils/workers';
import { getSynergies, getAgeTag, getFallbackResponse, getHealthSemantic, getStaminaSemantic, getTraumaSemantic } from '../shared/utils/gameLogic';
import { useEncounterBuffer } from '../shared/hooks/useEncounterBuffer';
import { saveGame } from '../shared/utils/saveManager';
import { resolveStoryEventStep, startStoryEvent } from '../shared/utils/storyEventEngine';
import { resolveEncounterAction } from '../shared/utils/encounterEngine';
import { resolveLocationAction, annotateActionsWithChance } from '../shared/utils/locationEventEngine';
import { FloatingDeltas } from '../features/ui/components/TextComponents';

// ── Lazy-loaded modals (only fetched when opened) ───────────────────────
const SaveLoadModal = React.lazy(() => import('../features/ui/components/SaveLoadModal').then(m => ({ default: m.SaveLoadModal })));
const JournalModal = React.lazy(() => import('../features/ui/modals/JournalModal').then(m => ({ default: m.JournalModal })));
const MapModal = React.lazy(() => import('../features/ui/modals/MapModal').then(m => ({ default: m.MapModal })));
const XRayModal = React.lazy(() => import('../features/ui/modals/XRayModal').then(m => ({ default: m.XRayModal })));
const StatsModal = React.lazy(() => import('../features/ui/modals/StatsModal').then(m => ({ default: m.StatsModal })));
const StatusModal = React.lazy(() => import('../features/ui/modals/StatusModal').then(m => ({ default: m.StatusModal })));
const MemoriesModal = React.lazy(() => import('../features/ui/modals/MemoriesModal').then(m => ({ default: m.MemoriesModal })));
const InventoryModal = React.lazy(() => import('../features/ui/modals/InventoryModal').then(m => ({ default: m.InventoryModal })));
const DeveloperModeModal = React.lazy(() => import('../features/ui/modals/DeveloperModeModal').then(m => ({ default: m.DeveloperModeModal })));
const SettingsModal = React.lazy(() => import('../features/ui/modals/SettingsModal').then(m => ({ default: m.SettingsModal })));
const ShopModal = React.lazy(() => import('../features/ui/modals/ShopModal').then(m => ({ default: m.ShopModal })));
const WardrobeModal = React.lazy(() => import('../features/ui/modals/WardrobeModal').then(m => ({ default: m.WardrobeModal })));
const SocialModal = React.lazy(() => import('../features/ui/modals/SocialModal').then(m => ({ default: m.SocialModal })));
const FeatsModal = React.lazy(() => import('../features/ui/modals/FeatsModal').then(m => ({ default: m.FeatsModal })));
const TraitsModal = React.lazy(() => import('../features/ui/modals/TraitsModal').then(m => ({ default: m.TraitsModal })));

// ── Lazy-loaded heavy components ────────────────────────────────────────
const CharacterModel = React.lazy(() => import('../features/rendering/3d/CharacterModel').then(m => ({ default: m.CharacterModel })));
const XRayView = React.lazy(() => import('../features/rendering/3d/XRayView').then(m => ({ default: m.XRayView })));
const NarrativePanel = React.lazy(() => import('../features/ui/components/NarrativePanel').then(m => ({ default: m.NarrativePanel })));
const ImmersiveStartMenu = React.lazy(() => import('../features/ui/components/ImmersiveStartMenu').then(m => ({ default: m.ImmersiveStartMenu })));
const EncounterUI = React.lazy(() => import('../features/ui/components/EncounterUI').then(m => ({ default: m.EncounterUI })));
const NarrativeLog = React.lazy(() => import('../features/ui/components/NarrativeLog').then(m => ({ default: m.NarrativeLog })));
const DoLStatsSidebar = React.lazy(() => import('../features/ui/components/DoLStatsSidebar').then(m => ({ default: m.DoLStatsSidebar })));

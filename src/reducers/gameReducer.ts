...
export function gameReducer(state: GameState, action: any): GameState {
  switch (action.type) {
    ...
    case 'RESOLVE_TEXT': {
      ...
      if (currentActivity === 'working' || intent === 'work') {
        hungerDrain += 2.0;
        energyDrain += 2.5;
        hygieneDrain += 1.5;
        happinessDrain += 0.5;
      ...
      if (currentActivity === 'sleeping') {
        ...
        hungerDrain *= 0.5;  // slower metabolism
        thirstDrain *= 0.5;
        happinessDrain = -2.0; // slight recovery
      }
      if (currentActivity === 'eating') {
        hungerDrain = -60.0;
        happinessDrain = -5.0;
      }
      if (currentActivity === 'bathing') {
        hygieneDrain = -50.0;
        happinessDrain = -3.0;
      }
      if (currentActivity === 'socializing' || intent === 'social') {
        socialDrain = -20.0;
        happinessDrain = -10.0;
      }

      newNeeds.hunger = Math.max(0, Math.min(100, newNeeds.hunger - hungerDrain * drainHours));
      newNeeds.thirst = Math.max(0, Math.min(100, newNeeds.thirst - thirstDrain * drainHours));
      newNeeds.energy = Math.max(0, Math.min(100, newNeeds.energy - energyDrain * drainHours));
      newNeeds.hygiene = Math.max(0, Math.min(100, newNeeds.hygiene - hygieneDrain * drainHours));
      newNeeds.social = Math.max(0, Math.min(100, newNeeds.social - socialDrain * drainHours));
      newNeeds.happiness = Math.max(0, Math.min(100, newNeeds.happiness - happinessDrain * drainHours));

      // 14b. Daily stats tracking
      const newDailyStats = { ...state.player.life_sim.daily_stats };
      newDailyStats.consecutive_awake_hours = currentActivity === 'sleeping' 
        ? 0 
        : newDailyStats.consecutive_awake_hours + drainHours;
      
      if (currentActivity === 'eating') newDailyStats.last_meal_time = state.world.hour;
      if (currentActivity === 'sleeping') newDailyStats.last_sleep_time = state.world.hour;
      
      // Calorie burn approximation
      newDailyStats.calories_burned += (hungerDrain * 20) * drainHours;

      // Activity remaining hours update
      let newActivityRemaining = Math.max(0, state.player.life_sim.schedule.activity_remaining_hours - drainHours);
      let newCurrentActivity = newActivityRemaining > 0 ? currentActivity : 'idle';
      ...
      if (newNeeds.hunger <= 20) {
        ...
        newStats.stamina = Math.max(0, newStats.stamina - 3 * drainHours);
        if (newNeeds.hunger <= 5) {
          newStats.health = Math.max(0, newStats.health - 5 * drainHours);
          newStats.willpower = Math.max(0, newStats.willpower - 2 * drainHours);
        }
      }
      if (newNeeds.energy <= 20) {
        newStats.willpower = Math.max(0, newStats.willpower - 3 * drainHours);
        newStats.stamina = Math.max(0, newStats.stamina - 2 * drainHours);
        if (newNeeds.energy <= 5) {
          newStats.hallucination = Math.min(100, newStats.hallucination + 5 * drainHours);
        }
      }
      if (newNeeds.hygiene <= 20) {
        newStats.allure = Math.max(0, newStats.allure - 5 * drainHours);
        newStats.stress = Math.min(100, newStats.stress + 2 * drainHours);
      }
      if (newNeeds.social <= 15) {
        newStats.stress = Math.min(100, newStats.stress + 3 * drainHours);
        newStats.willpower = Math.max(0, newStats.willpower - 1 * drainHours);
      }
      if (newNeeds.thirst <= 15) {
        newStats.health = Math.max(0, newStats.health - 4 * drainHours);
        newStats.stamina = Math.max(0, newStats.stamina - 6 * drainHours);
      }
      ...
      if (newDay > state.world.day) {
        ...
        newDailyStats.calories_burned = 0;
        newDailyStats.steps_taken = 0;
        newDailyStats.interactions_count = 0;
      }
      ...
      const newLifeSim = {
        ...
        schedule: {
          ...state.player.life_sim.schedule,
          current_activity: newCurrentActivity,
          activity_remaining_hours: newActivityRemaining
        }
      };
    ...
    case 'START_ACTIVITY': {
      const { activity, duration } = action.payload as { 
        activity: import('../types').LifeSim['schedule']['current_activity'], 
        duration: number 
      };
      return {
        ...state,
        player: {
          ...
          life_sim: {
            ...
            schedule: {
              ...
              activity_remaining_hours: duration
            }
          }
        }
      };

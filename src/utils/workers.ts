...
self.onmessage = function(e) {
  ...
  const cleanObject = (obj) => {
    if (Array.isArray(obj)) {
      ...
      const arr = obj.reduce<any[]>((acc, curr) => {
        const v = cleanObject(curr);
...
Active Quests: \${state.player.quests ? state.player.quests.reduce<string[]>((acc, q) => { if (q.status === 'active') acc.push(q.title); return acc; }, []).join(', ') || 'None' : 'None'}
\${translateLust(state.player.stats.lust)}
  Clothing: \${state.player.inventory.reduce<string[]>((acc, i) => { if (i.is_equipped) acc.push(\`\${i.name} (\${i.integrity}%)\`); return acc; }, []).join(', ') || 'Naked'}
...
export function buildImagePrompt(state: GameState) {
  ...
  const ascensionTag = (() => {
    switch (state.world.ascension_state) {
      case 'divine_spark': return "glowing soulfire aura";
      case 'daedric_champion': return "void halo, tendrils of shadow";
      case 'hist_devoted': return "fertile, nurturing gravitas";
      case 'sheogorath_touched': return "unsettling, fractured presence";
      default: return "";
    }
  })();

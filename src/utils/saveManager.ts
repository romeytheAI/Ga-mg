...
export function migrateGameState(rawState: unknown): GameState {
  ...
  return {
    ...
    world: {
      ...initialState.world,
      ...(world as any),
      current_location: resolveCurrentLocation(world as any),
      active_story_event: normalizeActiveStoryEvent((world as any).active_story_event),
      event_flags: { ...((world as any).event_flags || {}) },
      npc_relationships: Object.fromEntries(
        Object.entries((world as any).npc_relationships || {}).map(([id, rel]: [string, any]) => [
          id,
          {
      ...
      economy: { ...initialState.world.economy, ...((world as any).economy || {}) },
      ecology: { ...initialState.world.ecology, ...((world as any).ecology || {}) },
      factions: { ...initialState.world.factions, ...((world as any).factions || {}) },
      npc_state: { ...initialState.world.npc_state, ...((world as any).npc_state || {}) },
      meta_events: { ...initialState.world.meta_events, ...((world as any).meta_events || {}) },
      settlement: { ...initialState.world.settlement, ...((world as any).settlement || {}) },
      ambient: { ...initialState.world.ambient, ...((world as any).ambient || {}) },
    ...
    ui: {
      ...initialState.ui,
      ...(ui as any),
      horde_monitor: { ...initialState.ui.horde_monitor, ...((ui as any).horde_monitor || {}) },
      settings: { ...initialState.ui.settings, ...((ui as any).settings || {}) },
      graphics_quality: mergeGraphicsQuality((ui as any).graphics_quality),
    },
    memory_graph: (candidate as any).memory_graph || initialState.memory_graph,
    sim_world: (candidate as any).sim_world || initialState.sim_world,
    horde_queue: (candidate as any).horde_queue || initialState.horde_queue,
  };
}

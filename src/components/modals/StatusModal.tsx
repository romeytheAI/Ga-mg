...
export const StatusModal: React.FC<StatusModalProps> = ({ state, onClose }) => {
  ...
  const equippedStats = React.useMemo(() => {
    return state.player.inventory.reduce((acc, item) => {
      if (item.is_equipped) {
      ...
      return acc;
    }, { names: [] as string[], integritySum: 0, count: 0 });
    ...
    >
      ...
      >
        ...
        <div className="mt-8 pt-6 border-t border-white/10">
          ...
          <p className="text-sm text-white/80 font-serif italic">{equippedStats.names.join(', ') || 'Naked'}</p>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-[10px] tracking-widest uppercase text-white/40">Integrity</span>
            <span className="text-[10px] font-mono text-white/60">{Math.round(equippedStats.integritySum / (equippedStats.count || 1))}%</span>
          </div>
        </div>

...
export const StatusModal: React.FC<StatusModalProps> = ({ state, onClose }) => {
  // ⚡ Bolt: Using reduce to calculate equipment names and integrity in a single pass
  const equipmentStats = state.player.inventory.reduce(
    (acc, i) => {
      if (i.is_equipped) {
  ...
  const equipmentString = equipmentStats.names.join(', ') || 'Naked';
  const averageIntegrity = equipmentStats.count > 0 ? Math.round(equipmentStats.totalIntegrity / equipmentStats.count) : 0;
    ...
    >
      ...
      >
        ...
        <div className="mt-8 pt-6 border-t border-white/10">
          ...
          <p className="text-sm text-white/80 font-serif italic">{equipmentString}</p>

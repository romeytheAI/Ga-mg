...
export const StatusModal: React.FC<StatusModalProps> = ({ state, onClose }) => {
  // ⚡ Bolt: Using reduce to avoid multiple array allocations for equipped inventory
  const equippedSummary = React.useMemo(() => {
    return state.player.inventory.reduce<{ names: string[]; integritySum: number; count: number }>((acc, item) => {
      if (item.is_equipped) {
        acc.names.push(item.name);
        acc.integritySum += item.integrity || 0;
        acc.count += 1;
      }
      return acc;
    }, { names: [], integritySum: 0, count: 0 });
    ...
    >
      ...
      >
        ...
        <div className="mt-8 pt-6 border-t border-white/10">
          ...
          <p className="text-sm text-white/80 font-serif italic">{equippedSummary.names.join(', ') || 'Naked'}</p>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-[10px] tracking-widest uppercase text-white/40">Integrity</span>
            <span className="text-[10px] font-mono text-white/60">{Math.round(equippedSummary.integritySum / (equippedSummary.count || 1))}%</span>
          </div>
        </div>

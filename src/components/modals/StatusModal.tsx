...
export const StatusModal: React.FC<StatusModalProps> = ({ state, onClose }) => {
  // ⚡ Bolt: Using reduce to avoid multiple array allocations and caching the result
  const { equippedNames, averageIntegrity } = React.useMemo(() => {
    const stats = state.player.inventory.reduce<{names: string[], totalIntegrity: number, count: number}>((acc, item) => {
      if (item.is_equipped) {
      ...
      return acc;
    }, { names: [], totalIntegrity: 0, count: 0 });

    return {
      equippedNames: stats.names.join(', ') || 'Naked',
      averageIntegrity: Math.round(stats.count > 0 ? stats.totalIntegrity / stats.count : 0)
    };
    ...
    >
      ...
      >
        ...
        <div className="mt-8 pt-6 border-t border-white/10">
          ...
          <p className="text-sm text-white/80 font-serif italic">{equippedNames}</p>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-[10px] tracking-widest uppercase text-white/40">Integrity</span>
            <span className="text-[10px] font-mono text-white/60">{averageIntegrity}%</span>
          </div>
        </div>

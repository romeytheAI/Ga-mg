  ...
  return (
    <div className="relative group/sprite">
      ...
      {view3D ? (
        ...
        <div className="p-4">
          <CharacterSprite2D state={state} compact={false} showXRay={xrayOn} />
        </div>
      )}
...
export const StatsSidebar: React.FC<StatsSidebarProps> = React.memo(({
  ...
  // ⚡ Bolt: Using reduce to avoid multiple array allocations, memoized to prevent re-creation
  const equippedClothing = React.useMemo(() => {
    return clothingSlots.reduce<{slot: string, item: any}[]>((acc, slot) => {

...
function App({ state, dispatch }: { state: GameState, dispatch: React.Dispatch<any> }) {
  ...
  useEffect(() => {
    if (state.ui.last_stat_deltas && state.ui.haptics_enabled && (state.ui.last_stat_deltas.health ?? 0) < 0) {
  ...
  const handleAction = async (actionText: string, intent?: string, targetedPart?: string, actionId?: string) => {
    ...
    if (actionText === "Sleep and recover") {
      dispatch({ type: 'START_ACTIVITY', payload: { activity: 'sleeping', duration: 8 } });
      dispatch({ type: 'RESOLVE_TEXT', payload: { 
        parsedText: {
          narrative_text: "You find a quiet spot, wrap yourself in whatever warmth you can find, and sink into a deep, restorative slumber. The world fades away as your body begins to repair itself.",
          hours_passed: 8
        }, 
        actionText 
      } });
    ...
    if (actionText === "Find something to eat") {
      dispatch({ type: 'START_ACTIVITY', payload: { activity: 'eating', duration: 1 } });
      dispatch({ type: 'RESOLVE_TEXT', payload: { 
        parsedText: {
          narrative_text: "You take some time to consume whatever rations you have. The simple act of eating brings a small measure of comfort and strength back to your weary frame.",
          hours_passed: 1
        }, 
        actionText 
      } });
    ...
    if (actionText === "Wash yourself") {
      dispatch({ type: 'START_ACTIVITY', payload: { activity: 'bathing', duration: 1 } });
      dispatch({ type: 'RESOLVE_TEXT', payload: { 
        parsedText: {
          narrative_text: "Using what water you can find, you scrub away the grime of your journey. You feel refreshed and more presentable, though the water is bite-cold.",
          hours_passed: 1
        }, 
        actionText 
      } });

...
## 2025-02-28 - Information Leakage in Error Boundary and Firestore Error Handler
**Vulnerability:** The `ErrorBoundary.tsx` rendered the full stringified error object (including stack traces) to the DOM on a crash. Similarly, `src/firebase.ts` captured and logged highly sensitive PII (emails, tenant IDs, OAuth provider data) in a structured JSON object (`FirestoreErrorInfo`) on database errors, which was then thrown and potentially captured by telemetry or crash reporters.
**Learning:** React component crashes naturally surface stack traces during local development, but failing to sanitize the error component for production exposes internal code paths. Furthermore, aggressively capturing the full Firebase auth context directly into generic error handlers causes structural information leakage that propagates across the entire application stack.
**Prevention:** Always remove `authInfo` and PII from generic error handler structs. Use `console.error` to securely pass raw error objects alongside sanitized structures for developer visibility (`console.error("Msg", sanitized, rawErr)`) rather than stringifying PII. Implement a generic fail-safe fallback message in UI Error Boundaries.
